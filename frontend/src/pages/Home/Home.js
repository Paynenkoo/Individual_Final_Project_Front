...
export default function Home() {
  ...
  const following = Array.isArray(usersState.following) ? usersState.following : [];
  ...
  return (
    <div className={s.wrap}>
      <form className={s.searchBar} onSubmit={onSearch}>
        ...
        />
        <button className="btn-ghost" type="submit">Знайти</button>
      </form>

      {status === "loading" && <div className="notice">Завантажуємо…</div>}
      {status === "failed" && <div className="error">Помилка фіда</div>}
                  ...
                  <div className={s.followBtn}>
                    {followingNow ? (
                      <button
                        ...
                        onClick={() => dispatch(unfollowUser(String(p.authorId)))}
                      >
                        Відписатися
                      </button>
                    ) : (
